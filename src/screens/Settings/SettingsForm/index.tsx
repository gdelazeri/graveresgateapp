...
const SettingsForm = ({ navigation, route }: SettingsFormProps) => {
  ...
  const { list, setList, text, setText, isProcessing, isLoading, save } =
  ...
  const onPressDelete = (index: number) => {
    Alert.alert("Deseja realmente remover esse item?", "", [
      {
        text: "Não",
        style: "cancel",
      },
  ...
  return (
    <>
      ...
      />
      <Styled.Footer>
        <Button title="Salvar" onPress={onPressSave} loading={isProcessing} />
      </Styled.Footer>
    </>
  );
};
