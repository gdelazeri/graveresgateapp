...
import { postRescuerChecklist } from "@api/rescuerChecklist/rescuerChecklistApi";

export type PostRescuerChecklistPayloadField = keyof PostRescuerChecklistPayload;

export const useRescuerChecklistForm = () => {
  ...
  const [checklistQuestions, setChecklistQuestions] = useState<
    Checklist | undefined
  >();
  const [form, setForm] = useState<PostRescuerChecklistPayload>({});

  const setFormValue = useCallback(
    (key: PostRescuerChecklistPayloadField, value: string) => {
  ...
  const save = async () => {
    ...
    const response = await postRescuerChecklist(payload);
