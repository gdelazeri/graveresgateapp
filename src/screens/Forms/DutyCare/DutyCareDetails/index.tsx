...
const DutyCareDetails = ({ navigation, route }: DutyCareDetailsProps) => {
  ...
  const { isLoading, dutyCareChecklist } = useDutyCareDetails(id);
    ...
    <Styled.Container>
      <Styled.ScrollView>
        <CardInfo title="Informações gerais">
          ...
          <Label size={"medium"}>
            {moment(dutyCareChecklist.duty.date).format("LL")} -{" "}
            {DutyShiftLabel[dutyCareChecklist.duty.shift]}
          </Label>
          ...
          <Label size={"medium"}>
            {moment(dutyCareChecklist.date).format("DD/MM/YYYY")} às{" "}
            {dutyCareChecklist.time.substring(0, 5)}
          </Label>
          ...
          <Label size={"medium"}>
            {moment(dutyCareChecklist.createdAt).format("DD/MM/YYYY HH:mm")}
          </Label>
        ...
        <CardInfo title="Dados do paciente">
          ...
          <Label size={"medium"}>
            {VictimGenderLabel[dutyCareChecklist.victimGender as VictimGender]}
          </Label>
        ...
        <CardInfo title="Local da ocorrência/atendimento">
          ...
          <Label size={"medium"}>
            {dutyCareChecklist.incidentAddressDistrict}
          </Label>
          ...
          <Label size={"medium"}>
            {
              DutyCareChecklistIncidentContinuationLabel[
                dutyCareChecklist.incidentContinuation
              ]
            }
          </Label>
        </CardInfo>

        {Array.isArray(dutyCareChecklist.checklistFilledAnswers) &&
          dutyCareChecklist.checklistFilledAnswers.length > 0 && (
            <>
              <Styled.Divider />
              <ChecklistAnswers
