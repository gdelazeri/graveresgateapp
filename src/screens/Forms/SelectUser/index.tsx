...
const SelectUser = ({ navigation, route }: SelectUserProps) => {
  ...
  const { isLoading, searchValue, setSearchValue, list } = useSelectUser({
    position,
  });
  ...
  return (
    <>
      ...
      <FlatList
        data={list}
        renderItem={({ item }) => (
          <UserListItem user={item} onPress={() => onPressSelect(item)} />
        )}
        ItemSeparatorComponent={() => <Styled.DividerItem />}
        ListEmptyComponent={() => (
          <EmptyList text="Nenhum voluntário encontrado" />
        )}
      />
    </>
  );
