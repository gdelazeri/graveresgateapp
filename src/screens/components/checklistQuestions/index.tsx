...
const ChecklistQuestions = ({
  ...
  if (
    !checklistQuestions ||
    !Array.isArray(checklistQuestions.questions) ||
    checklistQuestions.questions.length === 0
  )
    return <></>;

  const renderQuestionInput = (question: ChecklistQuestion) => {
    const answer = form.checklistAnswers?.find(
      (answer) => answer.checklistQuestionId === question.id,
    );
    const selectedValue =
      question.multiple && isString(answer?.checklistQuestionOption)
        ? answer?.checklistQuestionOption.split(";")
        : answer?.checklistQuestionOption;
    ...
    switch (question.type) {
      case ChecklistQuestionType.TEXT:
        return (
          <Input
            ...
            onChangeText={(text) =>
              setFormChecklistQuestionValue({ question, optionValue: text })
            }
      ...
      case ChecklistQuestionType.OPTION:
        if (items.length > 0) {
          const answerItems = form.checklistAnswers?.filter(
            (answer) => answer.checklistQuestionId === question.id,
          );
          return (
            <RadioTableGroup
              ...
              selectedValue={answerItems?.map((answerItem) => ({
                item: answerItem.checklistQuestionItem as string,
                option: answerItem.checklistQuestionOption,
              }))}
              onChangeValue={(item, option) =>
                setFormChecklistQuestionValue({
                  question,
                  item,
                  optionValue: option,
