...
const useDutySelectUser = ({
  ...
  const [sectionList, setSectionList] = useState<
    { title: string; data: UserDutyRequest[] }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const requestedUsers = [
      ...list.filter((user) => requestedUserIds.includes(user.id)),
    ]
      .map((user) => {
        const dutyRequest = dutyRequests.find(
      ...
      .sort((a, b) =>
        a.dutyRequest?.createdAt > b.dutyRequest?.createdAt ? 1 : -1,
      );

    const otherUsers = list.filter(
      (user) => !requestedUserIds.includes(user.id),
    ) as UserDutyRequest[];
