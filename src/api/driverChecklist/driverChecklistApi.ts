...
export const postDriverChecklist = async (
  payload: PostDriverChecklistPayload,
) => {
  try {
...
export const getDriverChecklist = async (id: string) => {
  try {
    const response: AxiosResponse<ApiResponse<DriverChecklist>> = await get(
      `/v1/driver-checklist/get/${id}`,
    );
    return response.data;
  } catch (err) {
...
export const listDriverChecklist = async (page: number, pageSize: number) => {
  try {
    const response: AxiosResponse<ApiResponse<DriverChecklist[]>> = await get(
      `/v1/driver-checklist/list/paged`,
      { page, pageSize },
    );
    return response.data;
  } catch (err) {
